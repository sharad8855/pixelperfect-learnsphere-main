...
const MOCK_ACHIEVEMENTS = [
  {
    title: "Top Performer",
    sub: "Scored in top 10% in Database Quiz",
    date: "May 20, 2025",
  ...
  },
  {
    ...
    sub: "7 days of consistent learning",
    date: "May 18, 2025",
    tone: "bg-success/15 text-success",
    Icon: Zap,
  },
  {
    title: "Quick Learner",
    sub: "Completed 3 quizzes this week",
    date: "May 17, 2025",
    tone: "bg-warning/15 text-warning",
    Icon: Trophy,
  },
...
const MOCK_LIVE = [
  {
    ...
    month: "May",
    title: "Data Structures & Algorithms",
    prof: "Live Class with Prof. Neeraj Kumar",
    time: "10:00 AM",
    tone: "bg-primary/10 text-primary",
  },
  {
    ...
    month: "May",
    title: "Python Programming",
    prof: "Live Class with Prof. Ananya Verma",
    time: "02:00 PM",
    tone: "bg-success/15 text-success",
  },
  {
    ...
    prof: "Live Class with Prof. Priya Nair",
    time: "04:00 PM",
    tone: "bg-warning/15 text-warning",
  },
];
...
function Dashboard() {
  ...
  const viewRole = getRole();

  // Initialize Dashboard Mock Data locally
  useEffect(() => {
    ...
    const mockDashboardData = {
      testProgress: {
        courses: {
          ...
          completedByStudents: 28,
          pendingCompletion: 14,
        },
        tests: {
          created: 5,
          totalStudentsAssigned: 24,
          submittedByStudents: 18,
          reviewedByTutor: 16,
          pendingReview: 2,
        },
        actionItems: {
      ...
      <div className="min-w-0 space-y-6">
        ...
        {loading ? (
          <div className="flex h-64 items-center justify-center rounded-2xl bg-card border border-border">
            ...
            <span className="ml-2.5 text-sm font-semibold text-muted-foreground">
            ...
            {viewRole === "admin" ? (
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <StatCard
                  ...
                  value={String(
                    (dashboardData?.testProgress?.tests?.submittedByStudents ?? 18) +
                      (dashboardData?.testProgress?.actionItems?.submittedByStudents ?? 10),
                  )}
                  sub="Waiting Review"
              ...
              <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
                ...
                <StatCard
                  label="Completed"
                  value={String(
                    dashboardData?.recentCompletedActivity?.courses?.filter(
                      (c: any) => c.percentage >= 100,
                    ).length ?? 12,
                  )}
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                    ...
                    metrics={[
                      {
                        ...
                        value: dashboardData?.testProgress?.courses?.created ?? 8,
                      },
                      {
                        label: "Assigned Courses",
                        value: dashboardData?.testProgress?.courses?.assigned ?? 6,
                      },
                      {
                        label: "Total Enrollments",
                        value: dashboardData?.testProgress?.courses?.totalStudentsAssigned ?? 42,
                      },
                      {
                        label: "Completions",
                        value: dashboardData?.testProgress?.courses?.completedByStudents ?? 28,
                      },
                      {
                        label: "Pending",
                        value: dashboardData?.testProgress?.courses?.pendingCompletion ?? 14,
                      },
                    ]}
                  ...
                  <SummaryCard
                    ...
                    metrics={[
                      {
                        label: "Exams Created",
                        value: dashboardData?.testProgress?.tests?.created ?? 5,
                      },
                      {
                        label: "Total Assigned",
                        value: dashboardData?.testProgress?.tests?.totalStudentsAssigned ?? 24,
                      },
                      {
                        label: "Submissions",
                        value: dashboardData?.testProgress?.tests?.submittedByStudents ?? 18,
                      },
                      {
                        label: "Reviewed Status",
                        value: dashboardData?.testProgress?.tests?.reviewedByTutor ?? 16,
                      },
                      {
                        label: "Pending Grading",
                        value: dashboardData?.testProgress?.tests?.pendingReview ?? 2,
                      },
                    ]}
                  ...
                  <SummaryCard
                    ...
                    metrics={[
                      {
                        label: "Tasks Defined",
                        value: dashboardData?.testProgress?.actionItems?.created ?? 12,
                      },
                      {
                        label: "Student Mappings",
                        value:
                          dashboardData?.testProgress?.actionItems?.totalStudentsAssigned ?? 32,
                      },
                      {
                        label: "Submitted Tasks",
                        value: dashboardData?.testProgress?.actionItems?.submittedByStudents ?? 22,
                      },
                      {
                        label: "Approved Tasks",
                        value: dashboardData?.testProgress?.actionItems?.reviewedByTutor ?? 19,
                      },
                      {
                        label: "Pending Approval",
                        value: dashboardData?.testProgress?.actionItems?.pendingReview ?? 3,
                      },
                    ]}
              ...
              <section className="rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border">
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="rounded-md bg-primary/10 px-3 py-1 text-xs font-bold text-primary capitalize">
                      {viewRole}
                    </span>
                  </div>
                </div>
                ...
                <div className="space-y-3.5">
                  {MOCK_LIVE.map((l) => (
                    <div key={l.title} className="flex items-center gap-3">
                      <div
                        className={`flex w-12 flex-col items-center rounded-lg py-1.5 shrink-0 font-bold ${l.tone}`}
                      >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="truncate text-xs text-muted-foreground mt-0.5 font-medium">
                          {l.prof}
                        </div>
                      </div>
                      ...
                      <Button
                        size="sm"
                        className="rounded-lg font-semibold bg-primary text-primary-foreground hover:bg-primary/90 px-3 cursor-pointer"
                      >
                        Join
                      </Button>
                    </div>
            ...
            <div className="space-y-4 max-h-[300px] overflow-y-auto pr-1">
              {timelineData.length > 0 ? (
                timelineData.map((item, idx) => (
                  <div
                    key={idx}
                    className="flex items-start gap-2.5 text-xs pb-3.5 border-b border-border/40 last:border-0 last:pb-0"
                  >
                    ...
                    <div className="min-w-0 flex-1 leading-normal">
                      <p className="font-semibold text-foreground">
                        <span className="text-primary font-bold">{item.studentName}</span> submitted{" "}
                        <span className="font-bold">{item.title}</span> ({item.type}).
                      </p>
                      {item.marks && (
                        <p className="text-[10px] text-success font-bold mt-0.5">
                          Score/Marks: {item.marks}
                        </p>
                      )}
  ...
  return (
    <div className="min-w-0 rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border flex items-center gap-4">
      ...
      <div className="min-w-0 flex-1 leading-tight">
        ...
        <div
          className={`mt-1.5 truncate text-[11px] font-semibold flex items-center gap-1 ${subTone || "text-muted-foreground"}`}
        >
          {sub}
        </div>
      </div>
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-border bg-card shadow-sm hover:shadow-md transition-all duration-300 group">
      <div className="relative h-32 overflow-hidden bg-muted">
        ...
        <span className="absolute bottom-3 left-3 rounded bg-primary px-2 py-0.5 text-[10px] font-bold text-primary-foreground shadow-sm">
          In Progress
        </span>
      ...
      <div className="p-4">
        <div className="truncate text-sm font-bold text-foreground group-hover:text-primary transition-colors leading-tight">
          {title}
        </div>
  ...
  return (
    <section className="rounded-2xl bg-card p-5 shadow-sm ring-1 ring-border">
      ...
      <div className="mt-4 flex items-center justify-between px-1">
        <button className="rounded-md p-1 hover:bg-secondary cursor-pointer">
        ...
        {dates.map((d, i) => {
          ...
          return (
            <div key={i} className="flex items-center justify-center h-7 w-7 mx-auto">
              <div
                className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-semibold ${active ? "bg-primary font-bold text-primary-foreground shadow-md shadow-primary/20" : dim ? "text-muted-foreground/45" : "text-foreground hover:bg-secondary cursor-pointer"}`}
              >
                {d}
              </div>
            </div>
      ...
      <div className="space-y-3.5">
        {items.map((it) => (
          <div key={it.title} className="flex items-start gap-3">
            <div
              className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${it.tone}`}
            >
              <it.Icon className="h-[18px] w-[18px]" />
            </div>
            <div className="flex-1 min-w-0 leading-tight">
              <div className="truncate text-xs font-bold text-foreground">{it.title}</div>
              <div className="truncate text-[10px] text-muted-foreground mt-1 font-semibold">
                {it.sub}
              </div>
            </div>
            <div className="whitespace-nowrap text-[10px] font-bold text-muted-foreground/80 shrink-0 mt-0.5">
              {it.date}
            </div>
          </div>
        ))}
    ...
    <div className={`rounded-xl bg-card p-4 shadow-sm ring-1 ring-border leading-tight ${tone}`}>
      ...
      <div className="space-y-2.5 text-xs font-semibold text-muted-foreground">
        {metrics.map((m) => (
          <div
            key={m.label}
            className="flex justify-between items-center border-b border-border/40 pb-1.5 last:border-0 last:pb-0"
          >
